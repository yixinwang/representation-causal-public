# https://github.com/nmichlo/disent/blob/main/disent/metrics/_dci.py
# coding=utf-8
...
def metric_dci(
    ...
    Args:
      ground_truth_dataset: DisentDataset to be sampled from.
    ...
    mus_train, ys_train = disent_utils.generate_batch_factor_code(ground_truth_dataset, representation_function, num_train, batch_size, show_progress=False)
    assert mus_train.shape[1] == num_train
    assert ys_train.shape[1] == num_train
    mus_test, ys_test = disent_utils.generate_batch_factor_code(ground_truth_dataset, representation_function, num_test, batch_size, show_progress=False)
...
def _compute_importance_gbt(x_train, y_train, x_test, y_test, boost_mode='sklearn', show_progress=True):
    ...
    for i in tqdm(range(num_factors), disable=(not show_progress)):
        ...
        if len(np.unique(y_train[i,:])) > 1:
            ...
            importance_matrix[:, i] = np.zeros(num_codes)
            train_loss.append(1)
            test_loss.append(1)
