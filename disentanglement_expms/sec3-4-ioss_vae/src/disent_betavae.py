...
def metric_beta_vae(ground_truth_dataset, representation_function, random_state=npr.RandomState(0), artifact_dir=None, batch_size=64, num_train=10000, num_eval=5000):
    ...
    eval_points, eval_labels = _generate_training_batch(
            ground_truth_dataset, representation_function, batch_size, num_eval,
            random_state)
...
def _generate_training_batch(ground_truth_dataset, representation_function, batch_size, num_points, random_state):
    ...
    for i in range(num_points):
        labels[i], feature_vector = _generate_training_sample(
                ground_truth_dataset, representation_function, batch_size, random_state)
        if points is None:
...
def _generate_training_sample(ground_truth_dataset, representation_function, batch_size, random_state):
    ...
    index = random_state.randint(ground_truth_dataset._dataset.num_factors)
    # Sample two mini batches of latent variables.
    factors1 = ground_truth_dataset.ground_truth_data.sample_factors(batch_size, random_state)
    factors2 = ground_truth_dataset.ground_truth_data.sample_factors(batch_size, random_state)
    ...
    observation1 = ground_truth_dataset.dataset_batch_from_factors(factors1, mode='input')
    observation2 = ground_truth_dataset.dataset_batch_from_factors(factors2, mode='input')
    # Compute representations based on the observations.
    representation1 = to_numpy(representation_function(observation1))
    representation2 = to_numpy(representation_function(observation2))
